function func = stand_sim_gen (k, m, l0, g)
    func = @(t, Vars) stand_sim (t, Vars, k, m, l0, g);
end
...
% Vars(7) is x
% Vars(8) is y
function Res = stand_sim (t, Vars, k, m, l0, g)
    l_dot   = Vars(1);
    phi_dot = Vars(2);
    l       = Vars(3);
    phi     = Vars(4);
    ...
    Res(1) = d2_l_dt2(l_dot, l, phi_dot, phi, k, m, l0, g);
    Res(2) = d2_phi_dt2(l_dot, l, phi_dot, phi, k, m, l0, g);
    Res(3) = d_l_dt(l_dot, l, phi_dot, phi, k, m, l0);
    Res(4) = d_phi_dt(l_dot, l, phi_dot, phi, k, m, l0);
end

function res = d2_l_dt2 (l_dot, l, phi_dot, phi, k, m, l0, g)
    res = (l * phi_dot^2) - ((k/m) * (l-l0)) - g * cos(phi);
end

function res = d2_phi_dt2 (l_dot, l, phi_dot, phi, k, m, l0, g)
